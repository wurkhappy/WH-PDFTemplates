...
func agreement(params map[string]interface{}, body []byte) ([]byte, error, int) {
	...
	for _, payment := range payments {
		paymentTotal += payment["amountDue"].(float64)
	}
	...
	newAgreementTpl, _ := template.New("agreement_summary.html").Funcs(template.FuncMap{"formatDate": format}).ParseFiles(
...
func invoice(params map[string]interface{}, body []byte) ([]byte, error, int) {
	...
	var html bytes.Buffer
	err := newAgreementTpl.Execute(&html, m)
	log.Println(err)
	return html.Bytes(), nil, 200
}
